                        ...
                        <tbody>
                            {paginator.sales && paginator.sales.length > 0 ? (
                                paginator.sales.map(sale => (
                                    <tr key={sale.saleId}>
                                        <td>{sale.saleId}</td>
                                        <td>{sale.createdAt ? formatDate(sale.createdAt) : 'No Date Available'}</td>
                                        <td>{sale.totalValue !== null && sale.totalValue !== undefined
                                            ? sale.totalValue.toLocaleString('es-NP', { style: 'currency', currency: 'NPR' })
                                            : 'No Value'}</td>
                                        <td>{sale.customer ? sale.customer.name : 'No Customer'}</td>
                                        <td>{sale.user ? sale.user.name : 'No User'}</td>
                                        <td>
